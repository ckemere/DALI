"""
LLM-based code review for a single student submission.
...
Usage:
    from assess.code_review import review_submission
    results = review_submission("/path/to/extracted/submission")

Bulk review (multiple students in one API call) lives in
grading.bulk.lab1.review_bulk().
...
# Point value for each rubric item.
RUBRIC_POINTS = {
...
def collect_artifacts(submission_dir):
    ...
    the file path (for later upload via the Gemini Files API).
    """
    code_files = {}
    doc_files = {}
...
def review_submission(submission_dir, *, api_key=None, model=DEFAULT_MODEL,
                      verbose=False):
    """
    Send a single student submission to Gemini for code-review grading.
